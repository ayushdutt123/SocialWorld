...
import com.code.mysocial.databinding.ActivityProfileBinding;
        ...
        FirebaseFirestore.getInstance().collection("Users").document(FirebaseAuth.getInstance().getUid()).get().addOnSuccessListener(new OnSuccessListener<DocumentSnapshot>() {
            ...
            public void onSuccess(DocumentSnapshot documentSnapshot) {
                ...
                if (userModel.getUserProfile()!=null){
                    Glide.with(ProfileActivity.this).load(userModel.getUserProfile()).into(binding.userProfile);
                }
                if (userModel.getUserCover()!=null){
                    Glide.with(ProfileActivity.this).load(userModel.getUserCover()).into(binding.coverPhoto);
                }
            }
        });
    }
    private void loadPosts(){
        FirebaseFirestore.getInstance().collection("Posts").whereEqualTo("userId",userId).get()
